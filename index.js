...
client.once("ready", () => {
  ...
  if (!table2["count(*)"]) {
    sql
      .prepare(
        "CREATE TABLE guildhub (guild TEXT PRIMARY KEY, generalChannel TEXT, highlightChannel TEXT, muteChannel TEXT, logsChannel TEXT, streamChannel TEXT, reactionChannel TEXT, streamHere TEXT, autoMod TEXT, prefix TEXT);"
      )
      .run();
    sql
  ...
  if (!table3["count(*)"]) {
    sql
      .prepare("CREATE TABLE roles (guild TEXT, roles TEXT PRIMARY KEY);")
      .run();
  ...
  if (!table5["count(*)"]) {
    sql
      ...
      .run();
    sql
      .prepare("CREATE UNIQUE INDEX idx_wordguild_id ON words (wordguild);")
      .run();
    sql.pragma("synchronous = 1");
    sql.pragma("journal_mode = wal");
  }
  ...
  if (!table9["count(*)"]) {
    sql
      .prepare("CREATE TABLE usage (command TEXT PRIMARY KEY, number TEXT);")
      .run();
  ...
  try {
    ...
    console.log(
      nowtime +
        ...
        newMessage.guild.id +
        " " +
        newMessage.guild.owner.user.username +
        ": index.js:" +
        ln()
    );
  }
  ...
  if (message.author.id == "302050872383242240") {
    if (message.embeds[0].description.includes("Bump done")) {
      message.channel.fetchMessages().then(messages => {
        ...
        let userscore = client.getScore.get(dbumper[0], message.guild.id);
  ...
  if (message.member.hasPermission("KICK_MEMBERS")) {
    ...
    if (guildChannels.autoMod == "strict" || guildChannels.autoMod == "2") {
      ...
      if (message.mentions.users.size > 3) {
        message.delete();
        if (muteChannel1 == "0")
          return message.channel.send("You have not set up a mute channel!");
    ...
    if (message.channel.id === muteChannel1.id) {
      if (message.content == message.author.username + "1337") {
        if (guildChannels.autoMod == "strict") {
          ...
          let userscore1 = client.getScore.get(
            message.author.id,
            message.guild.id
          );
          if (!userscore1) {
  ...
  if (message.channel.id === "642882039372185609") {
    if (message.author.id !== "440892659264126997") {
      ...
      if (message.content.startsWith(prefix + "set")) {
        ...
        return message.channel.send(
          "Set channel id to: " + client.channels.get(`${cargs}`)
        );
      }
      let channelcheck = fs
        .readFileSync("channelset.txt")
