...
module.exports = {
    ...
    async execute(message) {
        ...
        ctx.fillText(user.username, 175, 100);
        ctx.font = '25px sans-serif';
        ctx.shadowColor = "black";
        ctx.shadowBlur = 5;
        ctx.fillStyle = '#ffffff';
        ctx.fillText(`Level: ${mathlev}\nPoints to next level: ${math3}`, 175, 135);
        if (mathwarn > 0) {
        ...
        const guildlogo = await Canvas.loadImage(user.displayAvatarURL);
